...
| `UNSUPPORTED_ON_BATCH_API` | A capability whose payload was never captured off the new UI, all on the Veo path: **4K video upscale**, **Veo r2v**, **Veo start+end-frame chaining**. Every Omni mode is ported, so this never names Omni any more | **Terminal — not retried** | For chaining and r2v, `FLOW_ALLOW_DEGRADED=1` falls back to plain i2v off the start frame. 4K upscale has no fallback — use 1080p. Real fix: capture the payload — `docs/CAPTURE.md` |
...
| `extension_connected: false` while `/health` is 200 and the log says `WebSocket server failed to bind` (or never says `WebSocket server listening`) | Port 9231 is taken by another process (`netstat -ano \| findstr :9231`) | Free the port, or set `WS_PORT` and change `AGENT_WS_URL` in `extension/background.js` to match. Note: the default used to be 9222, which Chrome DevTools remote debugging also uses — that is why it moved |
...
| Upscale fails on every scene | On the batch path only 1080p is ported (`p0UkFb`); a 4K request fails `UNSUPPORTED_ON_BATCH_API`. `no workflow id` means the clip's `jwpduf` record is gone — regenerate the video. On the legacy path it needs `PAYGATE_TIER_TWO` |
...
| Error | Cause | Fix |
|---|---|---|
| `Failed to review scene <id>: [WinError 2] The system cannot find the file specified` (every scene, right after `Creating contact sheets`) | `ffmpeg` is not on the API process's PATH. A winget install (`Gyan.FFmpeg`) puts it under `%LOCALAPPDATA%\Microsoft\WinGet\Packages\Gyan.FFmpeg_*\ffmpeg-*\bin` without registering PATH | `where ffmpeg` in a fresh shell. Add that `bin` folder to the user PATH and restart the API (`scripts\start.bat`); for one run, prepend it to `PATH` in the shell that launches the API |
